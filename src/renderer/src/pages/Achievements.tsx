            ...
            return (
              ...
              >
                ...
                {unlocked && achievement && (
                  <p className="text-xs text-text-muted mt-1">{format(new Date(achievement.unlocked_at.replace(' ', 'T')), 'dd/MM/yy')}</p>
                )}
              </div>
            )
          })}
          ...
          <div className="bg-bg-secondary border border-bg-border rounded-xl overflow-hidden">
            {profile.history.slice(0, 20).map((h, i) => (
              <div key={h.id}
                ...
                <span className="text-text-muted text-xs shrink-0">{format(new Date(h.created_at.replace(' ', 'T')), 'dd/MM HH:mm')}</span>
              </div>
            ))}
          </div>
        </div>
